...
// 1. 判断主机是否已经录入, 已经录入就选择更新, 未录入就保存
// save or update
func (s *service) SyncHost(ctx context.Context, ins *host.Host) (
	*host.Host, error) {
	exist, err := s.DescribeHost(ctx, host.NewDescribeHostRequestById(ins.Base.Id))
	if err != nil {
		// 如果不是Not Found则直接返回
...
func (s *service) UpdateHost(ctx context.Context, req *host.UpdateHostRequest) (
	*host.Host, error) {
	...
	// 查询出该条实例的数据
	ins, err := s.DescribeHost(ctx, host.NewDescribeHostRequestById(req.Id))
	if err != nil {
		return nil, err
	}
